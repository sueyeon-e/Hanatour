<%@page import="dao.TourTicketSearchDao"%>
<%@page import="dto.*"%>
...
<%
	...
	ArrayList<TourTicketSearchDto> listTourSearch = (ArrayList<TourTicketSearchDto>) request.getAttribute("listTourSearch");
	ArrayList<TourTicketSearchPriceDto> listTourPrice = (ArrayList<TourTicketSearchPriceDto>) request.getAttribute("listTourPrice");
	int countTourByCity = (Integer)request.getAttribute("countTourByCity");
...
<head>
	...
	<link rel="stylesheet" href="<%=request.getContextPath() %>/hanatour/css/tour_search.css">
		...
		<div id="main" class="fr">
			<div id="main_top">
				<div id="main_top_left" class="fl">
					...
					<!-- 여기 -->
					<span>(<%=request.getAttribute("countTourByCity")%>)</span>
				</div>
			...
			<div id="main_content">
				<div id="line">
					...
					<% for(TourTicketSearchDto dto : listTourSearch ) { %>
					<div class="content fl shadow" style="margin-left:0;">
						<a href="ControllerTour?command=tour_goods_detail&goodsIdx=<%=dto.getGoodsIdx() %>" name="goodsIdx" class="a_style" value="<%=dto.getGoodsIdx()%>">	
							<!-- 컨텐츠 이미지/설명 -->
							<div><img src="<%=dto.getFirstImgUrl() %>" class="content_img fl"></div>
							<div class="info fl">
								<div class="content_title truncate"><%=dto.getGoodsName() %></div>
								<div id="info_price" class="info_price">
								<% for(TourTicketSearchPriceDto priceDto :listTourPrice) { %>
									<% if(dto.getGoodsIdx()==priceDto.getGoodsIdx()) { %>
									<%
									    // 가격을 가져옴
									    double price = priceDto.getLowestPrice();
									    
									    // String.format을 사용하여 천 단위로 포맷팅
									    String formattedPrice = String.format("%,d", (long)price);
									%>
									<div class="price"><%= formattedPrice %><span>원</span></div>
									<% } %>
								<% } %>
									<div class="use_when">
									<% if(dto.getWhenUse()==null) {%>
									즉시 사용 가능
									<% } else { %>
									<%=dto.getWhenUse().split(" ")[0]%> 일부터 사용가능 
									<% } %>
									</div>
								</div>
								<div id="star" class="star_container"> 
									<div>
										<img src="별">
										<span class="review_count">(<%=dto.getReviewCount() %>)</span>
									</div>
								</div>
